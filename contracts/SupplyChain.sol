...
contract SupplyChain {
    ...
    uint internal skuCount; // SKU => Stock Keeping Unit
  ...
  /* Create a modifer that checks if the msg.sender is the owner of the contract */
    ...
    modifier forSale(State state) {
        require(state == State.ForSale, "Invalid State. Item not ForSale");
        _;
    }

    modifier sold(State state) {
        require(state == State.Sold, "Invalid State. Item is not Sold.");
        _;
    }

    modifier shipped(State state) {
        require(state == State.Shipped, "Invalid State. Item is not Shipped.");
        _;
    }

    modifier received(State state) {
        require(state == State.Received, "Invalid State. Item has not Received");
        _;
    }
